# Full replacement for @app.post("/api/auth/email") authenticate_email
...
def authenticate_email(payload: EmailAuthRequest):
    """Sign-up / sign-in with email. Password required for new accounts and verified when hash exists."""
    ...
    if user_id is not None:
        ...
        if stored_hash:
            ...
            # Migrate: first time password set on existing email-only account
            execute_write(
                """
                UPDATE app_users
